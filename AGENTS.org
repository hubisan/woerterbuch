...
3. For each AI task, AI creates/uses a task file in [[./.tasks/]] based on [[./.tasks/TEMPLATE.org]]: =.tasks/YYYY-MM-DD--slug.org=. Remove sections in the template if not relevant. Add other sections if it makes sense. Add the task start date at the top at ~#+TASK_STARTED:~ like [2026-06-02 Di]
5. AI links the task in [[./TODO.org]] to its task file. Example: =[[./.tasks/2026-05-24--anki-export-fix.org]]=
6. On =main=, create a focused branch, e.g. =feature/task-name= or =fix/task-name=; otherwise continue on the current branch and report suspicious branch/repo state.
...
9. AI updates docs/links, runs relevant checks.
10. AI documents results, checks, blockers, questions, follow-ups in the task file.
...
13. After approval by the user
    ...
    4. AI Adds completion date in task at top at ~#+TASK_COMPLETED:~ like [2026-06-02 Di].
    5. AI must not remove or archive the task entry in [[./TODO.org]] unless explicitly instructed.
14. AI commits only when explicitly asked.
15. Commits must be focused and have clear messages.
16. If asked to squash, AI reviews the branch diff, updates [[./CHANGELOG.org]] if needed, and creates one clear commit.
