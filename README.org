...
** AI Agent Instructions                                        :noexport_0:

This repository contains AI-specific development instructions in =AGENTS.org=. AI agents must read =AGENTS.org= before making changes.
...
For meanings and usage labels, Wiktionary is also intentionally parsed conservatively. If a qualifier such as =veraltet= or =reflexiv= is only embedded in the rendered sense text and not marked up reliably, it remains inside the definition text instead of being moved heuristically into a separate qualifier field.
